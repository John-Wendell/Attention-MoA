...
def get_cache_key(instruction: str, i_round: int) -> str:
    """Generate cache key"""
    key_str = f"{instruction}_{i_round}"
...
def process_fn(
    ...
    if len(references) == 0 and len(reference_models) > 0:
        ...
        for i_round in range(rounds):
            ...
            cache_key = get_cache_key(instruction, i_round)
            ...
            final_cache_key = get_cache_key(instruction + "_final", i_round)
            ...
            if i_round == 0:
                ...
                if len(history) < i_round:
                    ...
                    for prev_round in range(i_round):
                        if prev_round >= len(history):
                            prev_cache_key = get_cache_key(instruction + "_final", prev_round)
                            if cache_dir:
                ...
                # Generate res output
                res_cache_key = get_cache_key(instruction + "_res", i_round)
    ...
    return {"output": output, "generator": model + "-attention"}
